...
def readOrderFile():
    ...
    Returns: order_info
    '''
    # Read order file and put in a list
    order_info = []
    file_orders = open('orders.txt', 'r')
    for line in file_orders:
        line = line.rstrip()
        order_element = line.split('%') # seperate
        order_info.append(order_element)
    file_orders.close()
    return order_info
...
def readProductFile():
    ...
    '''
    product_info = []
    file_products = open('products.txt', 'r')
    for line in file_products:
        line = line.rstrip()
        product_element = line.split(';')
        product_info.append(product_element)
    file_products.close()
    return product_info
...
def readZoneFile():
    ...
    for line in zones:
        line = line.rstrip()
        zone_element = (line.split('#'))
        zone_element[1] = zone_element[1].split(',')
        zone_info.append(zone_element)
    zones.close()
    return zone_info
...
def createZoneDict(zone_info):
    ...
    for i in zone_info:
        ...
        val = i[1]
        zone_dict[key] = val
    keys = list(zone_dict.keys())
    values = list(zone_dict.values())
    return zone_dict
